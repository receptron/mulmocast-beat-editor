...
<template>
  <div class="flex h-full flex-col overflow-hidden">
    ...
    <component :is="'style'">
      ...
      0.15rem 0; } .beat-fragment code { background: #f5f5f4; padding: 0.1rem 0.3rem; border-radius: 0.2rem; } /* The swipe `elements` root is w-full h-full, so
      it collapses unless the host gives its PARENT a box — an inline height:100% beats any rule aimed at the root itself. mulmocast marks it
      data-mulmo-swipe-root (#1567) so this no longer has to match on that inline style. */ .beat-fragment:has(> [data-mulmo-swipe-root]) { height: 260px; }
    </component>

...
<template>
  <!--
    The container is a WRAPPER, not the flex row itself: an element's own `@3xl:` utilities
    resolve against its nearest ANCESTOR container, so `@container` and `@3xl:flex-row` on one
    element silently does nothing — measured, the pane's width switched at the breakpoint while
    the direction stayed column at every width.

    A container query, not a media query: the editor is embedded in hosts of every width — a
    plugin card of ~600px is the case this exists for — and what matters is the space THIS
    component was given, not the size of the window. The host declares nothing.

    Measured before this: at 600px the editing pane held its 384px and the list was left 216px,
    with 156px of that reaching the preview; at 480px the preview was 36px.
  -->
  <div class="@container h-full min-h-0">
    ...
    -->
      <component :is="'style'">
        ...
        } .beat-fragment h3 { font-size: 1.1rem; font-weight: 600; margin: 0.3rem 0; } .beat-fragment p { margin: 0.4rem 0; } .beat-fragment ul { list-style:
        disc; padding-left: 1.4rem; margin: 0.4rem 0; } .beat-fragment ol { list-style: decimal; padding-left: 1.4rem; margin: 0.4rem 0; } .beat-fragment li {
        margin: 0.1rem 0; } .beat-fragment code { background: #f5f5f4; padding: 0.1rem 0.3rem; border-radius: 0.2rem; } /* A chart card is sized for a slide
        (400px tall); in a list that is mostly empty space. */ .beat-fragment .chart-container > div { height: 220px !important; } .beat-fragment img,
        .beat-fragment video { max-height: 240px; width: auto; } /* A slide fragment is w-full h-full and designed at 1280px, so it needs a box to fill. */
        .beat-fragment > .relative.overflow-hidden { height: 300px; } /* The swipe `elements` root is w-full h-full, so it collapses unless the host gives its
        PARENT a box — an inline height:100% beats any rule aimed at the root itself. mulmocast marks it data-mulmo-swipe-root (#1567) so this no longer has to
        match on that inline style. */ .beat-fragment:has(> [data-mulmo-swipe-root]) { height: 260px; }
      </component>
      ...
      <!-- editor -->
      <!--
      Below the breakpoint the pane sits under the list and takes the full width; above it, the
      width is a token so a host can trade preview space for editing space without a fork.
    -->
      <aside
        class="max-h-1/2 w-full shrink-0 overflow-auto border-t border-stone-200 bg-white p-4 @3xl:max-h-none @3xl:w-[var(--beat-editor-pane-width,24rem)] @3xl:border-t-0 @3xl:border-l"
      >
